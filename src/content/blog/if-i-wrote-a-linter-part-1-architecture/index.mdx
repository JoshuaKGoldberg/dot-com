...
But, ESLint core rules are not type-aware.
...
FFI is tricky because Go functions work in a single shared memory space.
Users running the linter would be stuck with whatever version of TypeScript and Go is bundled with the linter.

Until Go's WebAssembly story is more favorable, I would use FFI for as fast type information as possible.
If users are running other Go code, they can always spawn a process for the linter.
...
If I wrote a linter, it would support all languages that are common to a supermajority of web projects:
