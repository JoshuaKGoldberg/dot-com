---
pubDate: 2025-05-04
description: "This is how I would architect a modern linter from scratch: focusing on built-in TypeScript support, a straightforward rule development experience, and deduplicating tooling."
image:
...
Unfortunately, the "one file at a time" model used by today's linters doesn't lend itself well to all the operations a codemod might need.
...
Once a usable cross-file cache is in place and re-running linting is much faster, we can unlock a fast `--watch` mode to help with it too.
