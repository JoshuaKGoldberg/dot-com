...
At the end of it, we'll have figured out how to take a template string literal like this one, simulate a Tic Tac Toe game on it, and determine out who the winner of that game is.
...
- Otherwise, we return a resultant type from `ApplyMoves`.
  That recursive type system call to `ApplyMoves` takes in the result of applying our first move to the board for a board, and all but the first move from our moves as its array of moves.
...
The last major type system feature we'll look at is also a new one, and is in my opinion one of the stranger –though still very useful– additions to TypeScript.
It's also only available on versions 4.1 later.
...
All concepts are new until you've had the time to play with them for a bit.
