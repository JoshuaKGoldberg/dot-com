---
description: "Native speed for parsing code is wonderful, but I think there are huge benefits to writing lint rules in the same language as the code they analyze. Here's why."
image:
...
I'd like to explain what those points mean, how they're different from the other linter archetypes, and the benefits of this approach.
...
TypeScript's Go rewrite, for example, benefitted heavily from Go's memory layout and allocation models [^typescript-go-discussion-why-go].

Those concerns are much less impactful for the areas of code that operate on already-created data structures.
Once

I set up a performance benchmark for a JIT speed linting: [lint-rule-jit-on-parsing-native-speed](https://github.com/JoshuaKGoldberg/lint-rule-jit-on-parsing-native-speed).
...
Consider too that typed linting often takes up 70% or more of the lint time for a file [^tseslint-pr-typescript-estree-jump-table].
Once native speed linting is unlocked with typescript-go [^tseslint-issue-typescript-go], files that today take 3 seconds would drop to (3 \* 0.3) + (3 \* 0.7 \* 0.1) = just under 1 second.
That's not including performance benefits from switching the parser to native speed as well.
...
In practice, I find developers don't mind typed linting performance as long as it don't noticeably slow down file saves or take noticeably longer than other CI tasks such as unit tests.
Those are the benchmarks I think we should prioritize measuring and optimizing for.
