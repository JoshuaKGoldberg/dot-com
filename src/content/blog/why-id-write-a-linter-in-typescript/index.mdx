...
I've seen plenty of projects from very capable team experiencing 2x [^cloudflare-workerd-pr-remove-eslint-prettier-config] slowdowns from entries covered in the [typescript-eslint Performance Troubleshooting & FAQs](https://typescript-eslint.io/troubleshooting/typed-linting/performance).

Furthermore, ESLint is not optimized for typed linting performance.
Its caching does not support cross-file information [^tseslint-docs-can-i-use-eslint-cache] and there are clear engine-level optimizations that have not been made [^eslint-rfc-parsing-session-objects].
...
That would be a huge shift in the web ecosystem and I don't see it happening any time soon.
I think the web ecosystem is going to continue to stay JavaScript/TypeScript-first for the forseeable future.
...
[Why I’m skeptical of rewriting JavaScript tools in “faster” languages](https://nolanlawson.com/2024/10/20/why-im-skeptical-of-rewriting-javascript-tools-in-faster-languages/) is a great post on the subject of native speed rewrites.
