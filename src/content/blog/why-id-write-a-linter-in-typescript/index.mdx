...
## TypeScript Over Native Speed Languages
...
Tools like Biome and Oxlint are truly wonderful and I think there's a lot of use for linting ginormous amounts of files with them.
Those are fantastic projects run by excellent teams, and they serve a real use case of ultra-fast tooling.

It is my belief that **the standard linter for an ecosystem should be written in a flavor of that ecosystem's primary language**.
Although other languages may be faster, the benefits for _developer_ and _ecosystem_ compatibility weigh heavily towards JavaScript or TypeScript for a web linter.
I think they'll still apply even when native speed linters are able to tap into TypeScript's type information.
...
Most professional teams developing web applications don't include many Rust-familiar developers.
...
But that also bifurcates the lint ecosystem: lint rule implementations will be split across multiple languages.
Any JavaScript/TypeScript developer who isn't confident in Rust will only be able to contribute to a portion of the linter's ecosystem.

I want users of my linter to be able to look at the source code of rules.
...
Consider again the case of a professional web development team.
Suppose they have TypeScript design system tokens and utilities that need to be referenced in custom lint rules, such as one that enforces migrating from old token values to new.
Lint rules written in Rust would need some interoperatbility layer to import and use those tokens, assuming those tokens match paradigms well-supported by Rust.
...
But their _conceptual_ overhead makes me lean strongly away from them.
...
ESLint's core rules don't understand type information, which is why typescript-eslint "extension" rules exist that add in type information [^tseslint-extension-rules-type-checked].
...
If a linter wants type checking with TypeScript today, it needs to create a TypeScript Program.
...
For lint rule authors, having to convert ESLint's native nodes to their TypeScript counterparts before passing to TypeScript APIs is an annoyance.
We've written utilities to help with common cases[^tseslint-pr-type-checker-wrapper-apis] but the conceptual overhead alone is painful.
...
ESLint's rules use a different AST structured, [ESTree](https://github.com/estree/estree), that is fundamentally different from TypeScript's AST structure.
...
I think this drawback is relevant, but not as pressing as it used to be.
...
For experimental evidence, see the [Oxlint Rules page](https://oxc.rs/docs/guide/usage/linter/rules.html) showing the _hundreds_ of community rules they've implemented.
...
#### Not A Drawback: Parsing Performance

I've seen a lot of developers express that the dual tree strategy comes with a performance penalty of generating two trees.
Although thinking about performance good, we should note that this is not a relevant performance concern for any project I've seen.
The time to compute typed linting for a project is always exponentially larger than the parse-and-convert of a dual tree.

For example, [Brad Zacher](https://github.com/bradzacher) tried out optimizing typescirpt-eslint's tree conversion logic.
The results there were that even with a 10% reduction in _text parsing_ time, the net reduction in _overall_ time was only ~0.2% [^tseslint-issue-conversion-logic-jump-table] [^tseslint-pr-typescript-estree-jump-table].
That was because lint rules themselves take up most lint timing.
...
We may be able to significantly speed up TypeScript type information in the longer term.
...
Maybe we could have a TypeScript [daemon](<https://en.wikipedia.org/wiki/Daemon_(computing)>) that preemptively generates type information.

Regardless, the important takeaway is that linters are easiest to contribute to when they utilize a single AST for each source file.
Performance considerations don't need to support that conclusion.
Developer ergonomics are, in my opinion, reason enough.
...
It saddens to discuss exasterbating that pain.
...
Here are the preferences I've stated so far:
...
- It's good to have multiple viable architectures of a linter be tried out

I don't have the time to write a new linter this year.
But I think there's a very promising architecture that should be tried out... eventually.
