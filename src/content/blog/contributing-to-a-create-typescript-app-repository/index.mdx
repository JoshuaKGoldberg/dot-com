...
This blog post is a comprehensive guide to the common activities you'll need to contribute to any of those repositories.
...
> 💡 Whenever you use a tool for the first time, **read the tool's documentation**.
> It will save you _so much time_ and make working much less confusing.
...
Unless you've been added as a collaborator or maintainer on someone else's repository, you won't have permissions to push branches to that repository on GitHub.
You'll need to:
...
At this point, the terms we'd use to refer to your repositories would be:
...
Two common situations for repositores are:
...
Once you've gotten as much confidence as you can in your changes, you'll want to send it in as a [Pull Request](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/about-pull-requests) for review.
It's best to create a separate branch for your changes instead of sending the pull request from your `main` branch.

I recommend giving the first commit on that branch a message adhering to [conventional commits].
...
You can always add more changes aferwards by making a new commit and pushing it.
The `-u origin` is no longer necessary for `git push` now that your local repostiory knows which remote the branch's commits should be pushed to:
...
If there are any comment threads you didn't resolve, such as from not knowing the right way to, post a respons to each specific comment thread.
...
> I'll move these into a separate article soon!
> Let me know: what other questions do you have?

- I'm having a hard time, how do I ask for help?
- Why are there so many steps?
  - also have note about many of these being implicit in other repos
- How confident in my changes should I be before sending a PR?
- I cloned a repository but now need to fork, how do I update?
- My files aren't formatted on (save|commit)?
- When should I use test snapshots?
- How do I deal with merge conficts?
- I sent a PR from `main`, how do I change the PR to be from a branch?
- You merged a PR sent from `main`, how do I reset `main` back to the upstream's?
- Why shouldn't I git force push
