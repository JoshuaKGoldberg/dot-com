---
description: "This is my standard explainer for pull request reviews where a try...catch statement is used more than I think it needs to be."
image:
...
There might be a request for changes linking to this blog post, asking that you avoid a the `try...catch`, or failing that, reduce its scope.

When as `try...catch` statement is unavoidable, I advise putting as little code as possible inside.
...
They catch _any_ exception, including ones that you might not expect them too.
...
Suppose you have a `emphasize` JavaScript function that's supposed to turn a string text into an uppercase version with an exclamation mark:
...
Let's say you're seeing errors from `emphasize` saying that `text.toUpperCase()` is not a function:
...
By refactoring to handle the "unhappy" case as early as possible, the code didn't have to worry about invalid data being passed to `emphasize`.
...
Any other unexpected error that occurs would fall back to however the project handles unexpected errors.
...
- [Exceptions as a form of control flow are inherently difficult to reason about](https://softwareengineering.stackexchange.com/questions/189222/are-exceptions-as-control-flow-considered-a-serious-antipattern-if-so-why)
- [Thrown errors can't be modeled by type systems like TypeScript's](https://www.learningtypescript.com/articles/why-typescript-doesnt-include-a-throws-keyword)

## Don't be afraid to ask for help

It's ok if you're having a hard time figuring out the right way to safely handle errors in code!
...
I'll be happy to give you pointers and work with you to write the code safely.
