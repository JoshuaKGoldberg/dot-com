...
The _**parsing**_ and _**type information**_ portions of it certainly should be for their incredible speed improvements.
...
The vast majority of time spent in JavaScript core linting is on _parsing_ and _type information_.
...
A properly configured linter should be running on files impacted by a change [^tseslint-docs-can-i-use-eslint-cache], which most of the time should not be a large percentage of your project.
...
Any native core linter that also allows JIT speed rule plugins will be stuck between a rock and a hard place:

- If they split core rules between JIT and native speed, then many end users will be slowed down to hybrid speed linting
- If they keep most or all core rules at native speed, JIT speed rules will have fewer references for end-users (and take extra maintenance time to document)
...
Once your lint rules are majority JIT speed, I don't see much benefit to writing the coordinating logic -arguments parsing, results printing, etc.- in native code.
The most significant performance bottlenecks, _parsing_ and _type information_ are already eliminated by using external native speed dependencies.
Prioritizing JIT speed for rules means linters are guaranteed to run at hybrid speed already.
...
2. Allowing JavaScript plugins slows native speed linters down to hybrid speed already.
3. JavaScript/TypeScript-first rules are more approachable for the majority of web developers.
4. Once lint rules are JIT speed, you might as well write coordinating logic in TypeScript as well.

In other words, I think an ideal web linter uses a JIT speed language for user-facing concerns and a native speed language for externally sourced dependencies:

| Concern          | Speed  | Why                                        |
| ---------------- | ------ | ------------------------------------------ |
| Coordination     | JIT    | More approachable; ecosystem compatibility |
| Lint Rules       | JIT    | More approachable; ecosystem compatibility |
| Parsing          | Native | Significantly better performance           |
...
I don't think any of that is happening anytime soon.
...
Thanks to the following reviewers for excellent advice and feedback on this post as well as general web linting: [auvred](https://github.com/auvred), [Brad Zacher](https://github.com/bradzacher), [Dimitri Mitropoulos (Michigan TypeScript)](https://michigantypescript.com), and [Joshua Chen](https://github.com/Josh-Cena).
I appreciate your insights and time!

<LabeledImage
	alt="Normal Rockwell's 'Tough Call': three baseball umpires look upward and one catches rain on a hand, as it starts to rain."
	description="None of those reviewers agree all of my conclusions here. Which is wonderful. Healthy discourse and trying out different approaches makes for a better ecosystem long-term. 🤗"
	original="https://en.wikipedia.org/wiki/Tough_Call#/media/File:Norman_Rockwell_-_Tough_Call.jpg"
	src={theGreatDebate}
/>
