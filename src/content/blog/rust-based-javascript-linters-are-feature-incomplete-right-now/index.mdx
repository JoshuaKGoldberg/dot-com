---
description: "Explaining why the marginal speed gains from Rust linters aren't comparable to the full feature set of typescript-eslint."
image:
...
title: "Rust-Based JavaScript Linters are Feature Incomplete Right Now"
---
...
The speedsups seen in tools such as [swc](https://swc.rs) and [Turbopack](https://turbo.build/pack) are very exciting for fast development experiences.
...
The idea of a linter that runs at Rust (native code) speed rather than JavaScript (JIT script) speed is quite appealing from the perspective of performance.
...
Traditionally, lint rules in linters such as ESLint only have visiblity into one source code file at a time.
This makes them fast and theoretically cacheable and parallizeable.
...
I know of two projects that are working on it: [Ezno](https://github.com/kaleidawave/ezno) and [`stc`](https://stc.dudy.dev).
...
> 💡 See [Ryan Cavanaugh's _Let's Make a Generic Inference Algorithm_ TypeScript Congress 2023 talk](https://portal.gitnation.org/contents/lets-make-a-generic-inference-algorithm) for an example of what TypeScript has to deal with.
...
As impressive as Ezno and `stc` are, their long-term feasiblility as standalone projects is precarious.
...
- Changing the way its checker runs to not rely on garbage collection, mutable objects, or other performance paper cuts as much
...
But it's going to be a very long time until that's feasable.
