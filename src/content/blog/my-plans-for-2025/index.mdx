---
pubDate: 2025-02-10
description: "How I'm going to make my 2024 impactful."
image:
  alt: "Meme cartoon of a dog sitting on a chair at a table with fire all around him."
  src: "~/assets/blog/this-is-fine.jpg"
title: "My Plans for 2025"
---

import homerSimpsonMirrorPecsFlexing from "~/assets/blog/homer-simpson-mirror-pecs-flexing.webm";
...
I posted my [2024 In Review](/blog/2024-in-review) blog a week ago, after procrastinating on it most of January.
...
<LabeledImage
	alt="Meme cartoon of a dog sitting on a chair at a table with fire all around him."
	description="Filling out 2025 goals by copy and pasting things I wanted to do in 2024. This is fine."
...
Each of the following sections is a key area I'm going to invest my time in for 2025, in decreasing order of time spent during working hours.

- [Open Source](#open-source)
- [Community Engagement](#community-engagement)
- [Personal](#personal)

I'm trying to give my self smaller and more attainable goals in 2025 than for past years.
There are more _Objective:_ notes in this blog post than 2023 or 2024, but the objectives themselves are generally granular and -I hope- more achievable.
...
typescript-eslint still represents the most user impact I make in open source.
...
>
> Objective: Publish at least 4 long-requested blog posts.

The landing page on [typescript-eslint.io](https://typescript-eslint.io) hasn't been reworked in several years.
It prioritizes explaining what ESLint and TypeScript are over the benefits or definition of typescript-eslint.
And that _"The tooling that enables ..."_ definition doesn't capture how essential the project is for _providing_ -not just _enabling_- TypeScript lint rules.

[@ElianCodes](https://github.com/ElianCodes) led me through a pairing to audit the site's docs earlier this year in [youtube.com/watch?v=Vyu8FU0VAqA](https://www.youtube.com/watch?v=Vyu8FU0VAqA).
I'd like to apply the suggestions from that audit to the typescript-eslint website.

Additionally, I've long wanted to flesh out resources on the site for high-impact areas of rules.
We got through some of that in 2024 but not all the ones I wanted.

In particular, I want us to have published at least 4 of:

- [typescript-eslint#6758 Docs: Add article about restrict-plus-operands & restrict-template-expressions best practices](https://github.com/typescript-eslint/typescript-eslint/issues/6758)
...
- [typescript-eslint#8139 Blog: Add post about allowAutomaticSingleRunInference](https://github.com/typescript-eslint/typescript-eslint/issues/8139)
- [typescript-eslint#9103 Docs: Write blog post on the history of ban-types, {}, and object/Object](https://github.com/typescript-eslint/typescript-eslint/issues/9103)
- [typescript-eslint#10364 Docs: Add blog post about history and usage of TypeScript's isTypeAssignableTo API](https://github.com/typescript-eslint/typescript-eslint/issues/10364)

Many of the longstanding community gripes about typescript-eslint and typed linting come in part from us not explaining ourselves well (see: [typescript-eslint#10666 Enhancement: [no-floating-promises] Provide a built-in SafePromise type](https://github.com/typescript-eslint/typescript-eslint/issues/10666)).
...
>
> Objective: Close out 20 of 26 issues from 2019 and 2020
...
It feels disorganized and I know it often results in real user requests getting buried.
...
If I consistently apply >15 hours a month to ESLint, and the work is an acceptable quality, I will be happy.
...
I'm on the Mocha maintainer team ([mocha#5027 📌 Project Status: Maintenance Reboot](https://github.com/mochajs/mocha/issues/5027)).
...
It's a much more modern website that also doesn't rely on very ancient versions of old packages.
...
The Yeoman ("yo") project is one of the classic pieces of web dev tooling.
It was _the_ generator framework when I started learning JavaScript.
Much like Mocha, it's no longer cutting edge and modern, but rather a stable legacy project to be kept afloat.
(Unlike Mocha, no significant alternative has captured significant market share -- more on that later...)
...
I'd learned Yeoman needed maintenance because I was researching repository templating engines.
I was researching repository templating engines because I wanted one that could work well for [create-typescript-app](https://github.com/JoshuaKGoldberg/create-typescript-app).
No existing templating engine comes close to what CTA needs and I think that's a real gap in web dev tooling.
...
It's such a cool system for swapping pieces of tooling in and out, where each piece can describe how it impacts other pieces.
I think most repository templates will want to start out with a more traditional [Handlebars](https://handlebarsjs.com)-based approach, and some will grow to the Blocks scale.
...
I think a big part of why no repository templating system has achieved Yeoman levels of adoption is because they haven't cracked the user story of keeping repositories updated over time.
...
But once it's released I'll continue to focus on improving the CTA repository experience.
...
Monorepos are an important facet of TypeScript development.
...
This section is copied over from [My Plans for 2024 > TypeStat (`ts-lift`)](/blog/my-plans-for-2024#typestat-ts-lift).
...
I'd explicitly listed [TypeStat as out of scope for 2023](/blog/my-plans-for-2023/#typestat).
...
I'd previously called this _"Personal Projects"_, but that's not the right name.
Since going full time open source I've mostly stopped working on coding projects in my personal time.
These projects are smaller-scale but still relevant to the web ecosystem.
...
It hit "recommended" feature parity with other linters in January 2024: meaning roughly all the rules you would want enabled by default were implemented.
But there are still some rules available in other package.json linters not yet available in `eslint-plugin-package-json`.
...
Native speed linters are starting to integrate typed linting, too: e.g. [Biome's new `noFloatingPromises` rule](https://github.com/biomejs/biome/pull/4911).
...
So, I plan on finally writing my own linter in 2025.
😱.

I'm not looking to try to replace ESLint or otherwise have the linter be part of the recommended create-typescript-app template.
I see this project as early-stage, experimental work.
But I'm very excited to try things out _my_ way.
...
[2024's plans for my packages](/blog/my-plans-for-2024/#other-packages) included onboarding those packages to `create-typescript-app`.
...
This year, as with 2023 and 2024, I'm going to continue being active online as a member of the developer community.
...
I've found it to be a consistently useful place to publish general TypeScript technical ideas that I can then reference in my personal blog and typescript-eslint's.
...
Now that we've run a handful of successful meetups we think we have a handle on the general tasks for each meetup.
We'd like to turn that knowledge into more predictable meetups: i.e. publishing the timing of talks and networking sessions ahead of time.

Thinking on organizing: every local meetup I've ever seen has burned out its lead organizers.
...
My goal for 2025 is that any organizer -including and especially myself- should be able to unexpectedly drop out of Boston TS Club without aversely affecting the meetups.
...
There are a lot of moving parts to contend with -- only some of which are during the conference days themselves.

2024 saw about 125 attendees seated in the theater during talks at peak during its one day.
2025 will be two days, so we can expect some deflation in attendance numbers.
But I'd still like to grow to 150 people in seats for a nice full-feeling theater.

I also would like to break even (read: not lose money) this year.
2024 came to a deficit of several thousand dollars.
...
Lastly, and perhaps most important for me personally, is that I don't want to burn out again in 2025.
...
I'm optimistic that by building a team of staff around us, as well as starting conference work several months earlier in the year, I'll avoid spending a month overeating ramen and re-watching _Mad Men_.
...
- Moving CSpell into ESLint: thereby removing the `cspell.json` file.
  I think the user experience of the spell checker necessitates editor suggestions that add words to the dictionary in the ESLint config file.
...
- Replacing tsup: [create-typescript-app#1910 🚀 Feature: Rethinking building: entry points, bundle: false, and tsup issues](https://github.com/JoshuaKGoldberg/create-typescript-app/issues/1910) tracks my discontent with the tsup tool.
  ...
  This would remove the `tsup.config.ts` file.
...
Many of them are smaller and more granular than 2024's, but I'm still nervous about getting them all done this year.
I _think_ the big splashy objectives are all doable as long as I focus on one each month or two and leave some overflow time at the end of the year:
...
<LabeledVideo
	alt="TODO"
	description="TODO"
	src={homerSimpsonMirrorPecsFlexing}
/>
