...
ESLint does not yet have a centralized listing [^eslint-issue-plugins-eslint-org] yet, so there isn't one canonical approach users should take.
...
[bingo-js > Example Repositories](https://github.com/bingo-js/#example-repositories) contains about a dozen repositories auto-generated from templates.
...
But those linters preserve ESLint's features and allow users to migrate their configs with minimal conceptual changes [^biome-guide-migrate-eslint-prettier].

My problem with two-way compatibility is twofold:

1. It binds the new linter to older decision decisions, even if they aren't best-in-class anymore (looking at you, errors vs. warnings...)
...
Taking errors vs. warnings as an example, the equivalent would be the new linter's "gradual onboardings" system [^goldblog-linter-part-2-only-errors].
I'm envisioning a system that allows users to make swathes of files as still onboarding to new rules.
...
Getting users to write -let alone maintain- a plugin for one linter is hard enough.
...
Getting users to maintain their configuration files for one linter -let alone understand mapping between multiple linters- is hard enough.
...
Next week will see the final entry in this blog post series.
