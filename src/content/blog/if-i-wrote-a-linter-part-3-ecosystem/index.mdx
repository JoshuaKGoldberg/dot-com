...
For the JavaScript and TypeScript language specifically, that includes taking rules from the following plugins:
...
If run in a project with existing dependencies, it would offer to add the plugins for those dependencies into the created configuration.
