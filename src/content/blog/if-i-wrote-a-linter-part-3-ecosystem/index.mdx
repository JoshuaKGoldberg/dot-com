...
This represents a difficult challenge for any new linter.
The more you "break" from the existing ESLint paradigms, the harder it is for users to migrate.
How much is reasonable to ask users to change?

I think most linters today are going for _two-way compatibility_: porting all of the main concepts from ESLint.
That includes my least favorite concept, errors vs. warnings [^goldblog-linter-part-2-only-errors].
...
If I wrote a linter, I would go for _one-way compatibility_.
...
The new system would, I think, could be made _better_ at supporting the use case.
...
[^eslint-issue-plugins-eslint-org]: [eslint/eslint#18824 Create plugins.eslint.org website](
