...
Our [`EXPERIMENTAL_useProjectService` option](https://typescript-eslint.io/packages/parser#experimental_useprojectservice) has started user testing and is showing a lot of promise.
...
##### typescript-eslint Committer Team Expansion

> Objective: Onboard two new consistently active committer team members.

Maintaining open source projects can be difficult and time-consuming... but if you can get more people to join you, it gets easier and less time-consuming!
...
- Promises: In particular [`@typescript-eslint/no-floating-promises`](https://typescript-eslint.io/rules/no-floating-promises).
  Our rules around asynchronous code are divisive: some users love the bug-catching; others think they're maniacally pedantic.
...
I joined the [ESLint commiter team](https://eslint.org/team) in January! 🥳
...
For my first year I just want to be a good member of the team and get used to the codebase, fellow committers, and existing processes.

There are a few issues I'm particularly interested in tackling ([#15559](https://github.com/eslint/eslint/issues/15559), [#17814](https://github.com/eslint/eslint/issues/17814)).
I hope I'll get to them, but don't want to commit to finishing anything so soon.
...
It's a venerable project with over a decade of common uses.
Although newer test frameworks have largely supplanted it for most new projects, Mocha still has a place in the ecosystem as a stable option used by many existing projects.
It's a fast, stable alternative that's higher-level than the new `node:test` and with fewer abstractions than Jest and Vitest.

Our goal is to keep Mocha afloat.
We don't plan on releasing any major breaking changes or earth-shattering new features.
We mostly and keep the project up with other test frameworks.
...
> Objective: Simplify the template to have 25% fewer root-level files.
>
> Objective: Make migrating an existing single-project or monorepo repository with divergent tooling needs a pleasant experience.
>
...
What started as a template for my personal files has turned into a reusable public use creator for general TypeScript projects.
...
I'd like to get it down to at most ~15 or so.
...
I'd like to make customization and monorepos first-class supported features in some way.
...
`create-typescript-app` uses a couple dedicated GitHub Actions to enforce some of those needs for now.
...
The project is a _difficult_ one to get right, and additionally needs some rearchitectures to work well.
I wasn't even sure what to call the thing!
...
It's not exactly ecosystem-shattering but it's really cute and makes it fun to add delighters to web apps - which I personally think is a great thing to do.
...
(Obviously, having DefinitelyTyped use my plugin would be beneficial for me personally. But I'd still want this even if it was someone else's plugin.)
...
Expect more from me on that soon!

### _(redacted)_

> Objective: Succesfully run _(redacted)_ without any major problems.

_(redacted)_

I've got a yearly planning point that's not _quite_ ready to be announced.
It'll likely be happening in October and I'll be making a ton of noise about it on social media.
Expect more from me on that soon! 😉
...
- It's fun! I like chatting with people on the internet - especially since not having real coworkers can be isolating.
...
You have to be very regular to get a real following doing it.
I would leave streams somewhat drained - despite being relatively less productive per hour than usual doing them.

For 2024, I've built up enough friends online and projects to work on that I can't reasonbly justify the investment in Twitch streaming.
So I'm putting it on pause for now.
😢
...
But I thnk 2024 is going to be even better.
